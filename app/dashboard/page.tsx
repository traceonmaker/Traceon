...
import {
  ...
  Euro, Briefcase, Receipt, Percent
} from 'lucide-react'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetch('/api/artisan/me').then(r=>r.json()).then(d=>{ if(d?.id) setArtisan(d) }).catch(()=>{})
  }, [])
  ...
  async function saveArtisan(fields: Partial<Artisan>) {
    const r = await fetch('/api/artisan/me', { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ id: artisan!.id, ...fields }) })
    ...
    <div style={{minHeight:'100vh',background:'var(--bg-grad)'}}>
      <div style={{maxWidth:480,margin:'0 auto',padding:'20px 16px 120px'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:18}}>
          ...
          <a href={`/formulaire/${artisan.id}`} className="fab" title="Lien client"><Link2 size={18} /></a>
        </div>
...
function Accueil({ today, nouvelles, encaisse, potentiel, confirmes, valider, validating, removing, onCreneaux, goTo }: any) {
  ...
  return (
    <div className="a-fadeUp">
      ...
      <div className="hero-card a-scaleIn" style={{padding:'22px 22px 20px',marginBottom:16}}>
        ...
        <div style={{position:'relative',zIndex:1}}>
          ...
          <div style={{display:'flex',gap:10,marginTop:18}}>
            <div className="hero-subcard" style={{flex:1,padding:'12px 14px'}}>
              <p style={{fontSize:11,fontWeight:600,color:'rgba(255,255,255,0.9)'}}>Potentiel</p>
              <p className="amount" style={{fontSize:19,marginTop:3}}>{formatPrix(potentiel)}</p>
            </div>
            <div className="hero-subcard" style={{flex:1,padding:'12px 14px'}}>
              <p style={{fontSize:11,fontWeight:600,color:'rgba(255,255,255,0.9)'}}>Aujourd'hui</p>
              <p style={{fontSize:19,fontWeight:800,marginTop:3,letterSpacing:'-0.01em'}}>{today.length} chantier{today.length>1?'s':''}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {today.length===0
        ? <Empty Icon={CalendarDays} title="Journée libre" sub="Aucun chantier prévu aujourd'hui." />
  ...
  return (
    <div className={`card ${removing?'card-validating':''}`} style={{padding:14,border:`1px solid ${highlight?'var(--blue-mid)':'var(--border)'}`,boxShadow:highlight?'var(--shadow)':'var(--shadow-sm)'}}>
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:'4px 14px',marginBottom:12}}>
        {c && <Meta Icon={Clock} txt={`${formatDate(c.date)} · ${formatHeure(c.heure_debut)}`} />}
        <Meta Icon={MapPin} txt={d.client_adresse} />
      </div>
      <div style={{display:'flex',gap:8}}>
        <a href={`tel:${d.client_telephone}`} className="fab"><Phone size={17} /></a>
